...
**Current (2026-09-14):** all **13/13** `biome.jungle.*` pieces are **ready** on G-Transfer evidence (`6293905`+). **Kit exit ≠ playable jungle** until compose emits real PackedScene instances (no `anvil_placeholder`) and `forge_scene` publishes with Godot open + honest `validation.json` `godot_import`. Helper DirectionalLights / empty WorldEnvironment are **replaced** by authored ProceduralSky + key/fill/rim (overcast) and god_rays shafts — still not baked lightmaps, still not AAA product exit. Phase (3) **product exit** still needs specialist clears (Quinn TA / Reed / Vale / Ori) — that is board exit, not index honesty. Ops (Remy/Nova) bars are separate from shipGate ready.
